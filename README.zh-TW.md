...
# 或 npm 全域安裝（需要先從 GitHub Release 下載）
...
# 填入你的 Telegram Bot Token 和 User ID
```

或使用互動式設定精靈：
```bash
npm run setup  # 或 ai-on-call setup（全域安裝後）
```
...
| npm 全域安裝 | `~/.ai-on-call/config.toml` |
| 自訂 | 設定 `AI_ON_CALL_CONFIG` 環境變數 |
...
| paypal | [![Donate with PayPal](https://img.shields.io/badge/Donate-PayPal-00457C?logo=paypal)](https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=4T7LWBH63XQEE) |
